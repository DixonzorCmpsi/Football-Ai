...
const MatchupView: React.FC<MatchupViewProps> = ({ week, home, away, onBack }) => {
  ...
  const processRoster = (roster: PlayerData[]) => {
    ...
    if (filterPos !== 'ALL') {
      processed = processed.filter(p => p.position === filterPos);
    }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col relative bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
        <button 
          onClick={onBack} 
          className="flex items-center text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-500 transition-colors"
        >
      ...
      <div className="mb-4 rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shrink-0">
        <MatchupBanner 
          ...
          overUnder={data.over_under}
          homeWinProb={data.home_win_prob}
          awayWinProb={data.away_win_prob}
        />
      </div>

      {/* 3. Main Layout: Rosters */}
      <div className="flex flex-1 min-h-0 relative overflow-visible">
            ...
            <div>
              ...
              <div className="space-y-3">
                {awayRoster.length > 0 ? (
                  ...
                  <div className="text-slate-400 text-sm italic py-4">No {filterPos} found.</div>
                )}
              </div>
            </div>
        ...
        {/* 4. Vertical Position Tabs (FIXED RIGHT) */}
        <div className="absolute right-0 -mr-6 top-1/2 -translate-y-1/2 flex flex-col gap-0 z-50">
          {(['ALL', 'QB', 'RB', 'WR', 'TE'] as PositionFilter[]).map((pos) => (
            <button 
              ...
              className={`
                h-16 w-10 text-[10px] font-black tracking-widest flex items-center justify-center transition-all duration-200 
                border-y border-l rounded-l-lg border-r-0 shadow-sm
                ${filterPos === pos 
                  ? 'bg-blue-600 text-white border-blue-500 w-12 shadow-lg z-20' 
                  : 'bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 z-10'}
                [writing-mode:vertical-rl] rotate-180 mb-[-1px]
              `}
            >
              {pos}
            </button>
