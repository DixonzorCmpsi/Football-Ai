...
const LiveScoresBar: React.FC<LiveScoresBarProps> = ({ week, onGameClick }) => {
  ...
  if (loading && games.length === 0) {
    return (
      <div className="bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-white px-4 py-2 flex items-center justify-center gap-2">
        <div className="w-4 h-4 border-2 border-blue-500 dark:border-blue-400 border-t-transparent rounded-full animate-spin"></div>
        <span className="text-xs text-slate-500 dark:text-slate-400">Loading live scores...</span>
      </div>
    );
  }
  ...
  if (error && games.length === 0) {
    return (
      <div className="bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-white px-4 py-2 flex items-center justify-center gap-2">
        ...
        <button onClick={handleRefresh} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-800 rounded">
  ...
  return (
    <div className="bg-gradient-to-r from-slate-100 via-slate-50 to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-700/50">
      <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5">
        {/* Live indicator */}
        <div className="flex items-center gap-1 sm:gap-1.5 pr-2 sm:pr-3 border-r border-slate-300 dark:border-slate-700 shrink-0">
          {hasLiveGames ? (
            <>
              <Circle size={8} className="text-red-500 fill-red-500 animate-pulse" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-red-500 dark:text-red-400 hidden sm:inline">LIVE</span>
            </>
          ) : (
            <>
              <Radio size={12} className="text-slate-400 dark:text-slate-500" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-500">Wk {week}</span>
            </>
          )}
        </div>
        ...
        <button 
          ...
          className={`p-1 rounded transition-colors shrink-0 ${canScrollLeft ? 'hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300' : 'text-slate-300 dark:text-slate-700 cursor-not-allowed'}`}
        >
          <ChevronLeft size={16} />
        </button>
        ...
        <div className="flex-1 flex items-center gap-2 sm:gap-3 overflow-hidden justify-center sm:justify-start">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1 sm:gap-1.5">
                  ...
                  />
                  <span className={`text-[10px] sm:text-xs font-bold ${awayWinning ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                    {game.away_team}
                  </span>
                  {game.away_score !== null && (
                    <span className={`text-xs sm:text-sm font-black tabular-nums ${awayWinning ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>
                      {game.away_score}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1 sm:gap-1.5">
                  ...
                  />
                  <span className={`text-[10px] sm:text-xs font-bold ${homeWinning ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                    {game.home_team}
                  </span>
                  {game.home_score !== null && (
                    <span className={`text-xs sm:text-sm font-black tabular-nums ${homeWinning ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>
                      {game.home_score}
                    </span>
                  )}
                </div>
          ...
          {games.length === 0 && (
            <span className="text-xs text-slate-400 dark:text-slate-500 italic">No games scheduled</span>
          )}
        </div>

        {/* Game counter for mobile */}
        <span className="text-[10px] text-slate-500 dark:text-slate-500 sm:hidden shrink-0">
        ...
        <button 
          ...
          className={`p-1 rounded transition-colors shrink-0 ${canScrollRight ? 'hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300' : 'text-slate-300 dark:text-slate-700 cursor-not-allowed'}`}
        >
        ...
        <button 
          ...
          className="p-1 sm:p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition-colors border-l border-slate-300 dark:border-slate-700 ml-1 sm:ml-2 pl-2 sm:pl-3 shrink-0"
