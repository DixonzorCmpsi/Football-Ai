...
import polars as pl
import numpy as np
import nflreadpy as nfl # This script needs nflreadpy for the player map
...
def generate_features_all(
    ...
    pfr_player_id = None
    if df_players_map is not None:
        pfr_map = df_players_map.filter(pl.col('gsis_id') == player_id).select('pfr_id')
        if not pfr_map.is_empty(): pfr_player_id = pfr_map.item()

    player_history = player_history_stats # Default
    if pfr_player_id and df_snap_counts is not None:
        snaps = df_snap_counts.filter((pl.col('pfr_player_id') == pfr_player_id) & (pl.col('season') == CURRENT_SEASON) & (pl.col('week') < target_week))
    ...
    # These are the *current week* features the model expects
    features['age'] = age
    features['years_exp'] = years_exp
    features['draft_ovr'] = draft_ovr
    # Placeholders for features we couldn't easily get from RAG data
    ...
    # This is the "superset" of all player stats your models might need
    player_cols_to_lag = [
    ...
    # --- C: Opponent Rolling Averages (Current Matchup) ---
    if not opponent_defense_history.is_empty():
        last_def_row = opponent_defense_history.row(0, named=True)
        def_cols = [
            'rolling_avg_points_allowed_4_weeks', 'rolling_avg_passing_yards_allowed_4_weeks',
            'rolling_avg_rushing_yards_allowed_4_weeks', 'rolling_avg_sack_4_weeks',
            'rolling_avg_interception_4_weeks', 'rolling_avg_qb_hit_4_weeks',
            'rolling_avg_points_allowed_to_QB', 'rolling_avg_points_allowed_to_RB',
            'rolling_avg_points_allowed_to_WR', 'rolling_avg_points_allowed_to_TE'
        ]
        for col in def_cols:
            features[col] = last_def_row.get(col, 0.0)
    else:
        features['rolling_avg_points_allowed_4_weeks'] = 22.0 # Fallback for Week 1
        # All others will default to 0.0, which is fine
        
    # --- D: One-Hot Position (for RB/TE/WR models) ---
