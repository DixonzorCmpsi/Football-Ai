...
OUTPUT_CSV_PATH = Path("weekly_modeling_dataset_avg.csv")
...
def engineer_rolling_defense_features(team_offense_df, team_defense_df, opponents_df, player_weekly_df):
    logging.info("Engineering rolling features + lags for opponent defenses...")
    ...
    # Part A: General Offensive Stats Allowed (Points, Yards)
    ...
    # Part B: Direct Defensive Stats (Sacks, Ints)
    ...
    # Part D: Calculate Rolling Averages AND Explicit Lags
    ...
    # Track which columns we want to keep
    cols_to_keep = ['season', 'week', 'team']
    ...
    for stat in stats_to_roll:
        if stat in def_merged_df.columns:
            ...
            # 1. Rolling Average
            roll_col = f'rolling_avg_{stat}_4_weeks'
            def_merged_df[roll_col] = def_merged_df.groupby('team')[stat].shift(1).rolling(4, min_periods=1).mean()
            cols_to_keep.append(roll_col)
            
            # 2. Explicit Lags (Last week, 2 weeks ago, 3 weeks ago)
            for lag in [1, 2, 3]:
                lag_col = f'opp_def_{stat}_lag_{lag}'
                def_merged_df[lag_col] = def_merged_df.groupby('team')[stat].shift(lag).fillna(0)
                cols_to_keep.append(lag_col)

    # Return only the engineering features + keys
    return def_merged_df[cols_to_keep].copy()

def engineer_rolling_opp_offense_features(team_offense_df):
    """
    Calculates Opponent Offense Stats (Rolling + Lags).
    High opponent offense -> likely shootout -> game script features.
    """
    logging.info("Engineering rolling features + lags for opponent OFFENSES...")
    if team_offense_df.empty: return pd.DataFrame()

    # Normalization
    if 'team_abbr' in team_offense_df.columns: 
        df = team_offense_df.rename(columns={'team_abbr': 'team'}).copy()
    else:
        df = team_offense_df.copy()
        
    df = df.sort_values(['team', 'season', 'week'])
    
    # Metrics to track for game script
    # 'total_off_points' comes from standardize_columns
    metrics = ['total_off_points', 'total_yards', 'passing_yards', 'rushing_yards']
    cols_to_keep = ['season', 'week', 'team']

    for col in metrics:
        if col in df.columns:
            # 1. Rolling Average
            roll_col = f'opp_off_rolling_{col}_4_weeks'
            df[roll_col] = df.groupby('team')[col].transform(lambda x: x.shift(1).rolling(window=4, min_periods=1).mean()).fillna(0)
            cols_to_keep.append(roll_col)

            # 2. Explicit Lags
            for lag in [1, 2, 3]:
                lag_col = f'opp_off_{col}_lag_{lag}'
                df[lag_col] = df.groupby('team')[col].shift(lag).fillna(0)
                cols_to_keep.append(lag_col)
...
def combine_datasets(dataframes: dict, opponents_df: pd.DataFrame, defense_features_df: pd.DataFrame, offense_features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 2. Add Opponent Defense Stats
    # Note: defense_features_df has 'team', which corresponds to the 'opponent' in master_df
    defense_features_df.rename(columns={'team': 'opponent'}, inplace=True)
    master_df = pd.merge(master_df, defense_features_df, on=['season', 'week', 'opponent'], how='left')
    
    # 3. Add Opponent Offense Stats (New)
    offense_features_df.rename(columns={'team': 'opponent'}, inplace=True)
    master_df = pd.merge(master_df, offense_features_df, on=['season', 'week', 'opponent'], how='left')
...
def main():
    ...
    # Engineer Features
    rolling_def_feats = engineer_rolling_defense_features(
        all_data['team_offense_weekly'], 
        all_data['team_defense_weekly'], 
        opponents, 
        all_data['player_weekly']
    )

    rolling_off_feats = engineer_rolling_opp_offense_features(
        all_data['team_offense_weekly']
    )
    ...
    # Combine
    master_dataset = combine_datasets(all_data, opponents, rolling_def_feats, rolling_off_feats)
